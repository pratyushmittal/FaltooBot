...
def test_sub_agent_instructions_include_portable_shell_guidance(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(instructions.Path, "home", lambda: tmp_path / "home")
    config = Config(
    ...
    result = instructions.get_system_instructions(
        config, "sub-agent@test", workspace
    )

    assert "prefer `python3` over `python`" in result
    assert "Do not assume optional commands or Python packages are installed" in result

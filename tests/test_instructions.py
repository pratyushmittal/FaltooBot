...
def test_get_system_instructions_uses_group_prompt(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(instructions.Path, "home", lambda: tmp_path / "home")
    config = Config(
        home=tmp_path,
        root=tmp_path / ".faltoobot",
    ...
    result = instructions.get_system_instructions(
        config, "120363000000000000@g.us", workspace
    )

    assert "multi-person WhatsApp group chat" in result
    assert "Use recent group history as context" in result

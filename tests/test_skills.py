...
def test_bundled_google_places_skill() -> None:
    ...
    assert "description: Use Google Places API" in text
    assert "GOOGLE_MAPS_API_KEY" in text
    assert "PlacesClient" in text
    assert "## Search Places" in text
    assert "## Place Details" in text
    assert "SearchTextRequest" in text
    assert "GetPlaceRequest" in text
    assert "x-goog-fieldmask" in text
    assert "current WhatsApp location" in text
    assert "Places API is not configured" in text
    assert "ask whether they want to set it up" in text
    assert "web search/general knowledge results" in text
    assert 'raise SystemExit("Google Places is not configured' not in text
